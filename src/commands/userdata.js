...
module.exports = {
    ...
    async execute(interaction) {
        ...
        .then(json => {
			...
			interaction.editReply({ embeds: [new Discord.EmbedBuilder().setTitle(`User data for ${user}`).addFields('Expiry:', `${expiry}`).addFields('Subscription name:', `${subscription}`).addFields('Last Login:', `${lastlogin}`).addFields('HWID:', `${hwid}`).addFields('Created On:', `<t:${json['createdate']}:f>`).addFields('IP Address:', `${ip}`).setColor(Colors.Blue).setTimestamp()], ephemeral: true})
        })
    },
};