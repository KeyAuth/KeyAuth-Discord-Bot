...
module.exports = {
    ...
    async execute(interaction) {
		...
		if(amount > 20) return interaction.reply({ embeds: [new Discord.MessageEmbed().setTitle('Failure').addField('Reason:', `You cannot add more than twenty keys at a time.`).setColor("RED").setFooter({ text: "KeyAuth Discord Bot" }).setTimestamp()], ephemeral: true})
        ...
        if(days) {
			...
			.then(text => {
				...
				{
					interaction.reply({ embeds: [new Discord.MessageEmbed().setTitle('Key(s) Successfully Added!').addField('Key(s) Added:', `${text}`).setColor("GREEN").setTimestamp()], ephemeral: true})
			...
			if(licenseAdd === null) return interaction.reply({ embeds: [new Discord.MessageEmbed().setDescription(`No config saved for adding licenses yet. Please do a command with paramaters included then this will work.`).setColor("RED").setTimestamp()], ephemeral: true})
