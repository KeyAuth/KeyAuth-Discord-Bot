...
module.exports = {
    ...
    async execute(interaction) {
        ...
        .then(json => {
			if (json.success)
            {
				interaction.editReply({ embeds: [new Discord.EmbedBuilder().setTitle(json.message).addFields([{name: 'User Extended:', value: `${un}`}]).setColor(Colors.Green).setTimestamp()], ephemeral: ephemeral})
			} else {
