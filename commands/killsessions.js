...
module.exports = {
    ...
    async execute(interaction) {
        ...
        .then(json => {
            if (json.success)
            {
                interaction.editReply({ embeds: [new Discord.EmbedBuilder().setTitle("Successfully Killed All Sessions").setColor(Colors.Green).setTimestamp().setFooter({ text: "mazkdevf_bot Discord Bot" })], ephemeral: ephemeral})
            } else {
