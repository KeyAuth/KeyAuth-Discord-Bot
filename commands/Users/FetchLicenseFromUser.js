        ...
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=getkey&user=${un}`)
            ...
            .then(json => {
                if (json.success) {
                    ...
                    interaction.editReply({ embeds: [new EmbedBuilder().setTitle(`Failure`).addFields([{ name: 'Note:', value: `Your seller key is most likely invalid. Change your seller key with \`/add-application\` command.` }]).setColor(Colors.Red).setFooter({ text: "KeyAuth Discord Bot" }).setTimestamp()], ephemeral: ephemeral })
                }
