...
module.exports = {
    ...
    async execute(interaction) {
        ...
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=verifyuser&user=${user}`)
            ...
            .then(json => {
                if (json.success) {
                    interaction.editReply({ embeds: [new Discord.EmbedBuilder().setTitle(json.message).addFields([{ name: 'User exists:', value: `\`${user}\``}]).setColor(Colors.Green).setTimestamp()], ephemeral: ephemeral })
                }
                else {
