...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .addStringOption((option) =>
            option
                .setName("user")
                .setDescription("The username of the user you would like to apply the cooldown for.")
                .setRequired(true)
        )
        .addIntegerOption((option) =>
            option
                .setName("cooldown")
                .setDescription("The duration of the cooldown in seconds. Default 120 seconds.")
                .setRequired(true)
        ),
    async execute(interaction) {
        ...
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=setcooldown&user=${user}&cooldown=${cooldown}`)
            .then(res => res.json())
            .then(json => {
                if (json.success) {
