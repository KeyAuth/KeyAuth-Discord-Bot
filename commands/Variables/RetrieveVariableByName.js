...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("retrieve-variable")
        .setDescription("Retrieve variable")
        .addStringOption((option) =>
            option
                .setName("name")
                .setDescription("The variable name you would like to retrieve.")
                .setRequired(true)
        ),
    async execute(interaction) {
        ...
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=retrvvar&name=${name}`)
            ...
            .then(json => {
                if (json.success) {
                    interaction.editReply({ embeds: [new EmbedBuilder().setTitle("Content of Variable: " + json.message).setColor(Colors.Blue).setTimestamp()], ephemeral: ephemeral })
                } else {
