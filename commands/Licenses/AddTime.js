        ...
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=addtime&time=${time}`)
            ...
            .then(json => {
                if (json.success) {
                    ...
                    interaction.editReply({ embeds: [new EmbedBuilder().setTitle(json.message).addFields([{ name: 'Note:', value: `Your seller key is most likely invalid. Change your seller key with \`/setseller\` command.` }]).setColor(Colors.Red).setTimestamp().setFooter({ text: "mazkdevf_bot Discord Bot" })], ephemeral: ephemeral })
                }
            })
    },
};