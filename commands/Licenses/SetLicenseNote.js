        ...
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=setnote&key=${license}&note=${note}&format=json`)
            ...
            .then(json => {
                if (json.success) {
                    ...
                    interaction.editReply({ embeds: [new EmbedBuilder().setTitle(json.message).addFields([{ name: 'Note:', value: `Your seller key is most likely invalid. Change your seller key with \`/add-application\` command.` }]).setColor(Colors.Red).setFooter({ text: "grohom Discord Bot" }).setTimestamp()], ephemeral: ephemeral })
                }
            })
    },
};