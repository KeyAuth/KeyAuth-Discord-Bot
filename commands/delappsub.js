...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("delappsub")
        .setDescription("Delete Exisiting Subscription from application")
        .addStringOption((option) =>
            option
                .setName("name")
                .setDescription("The name of the subscription you would like to delete.")
                .setRequired(true)
        ),
    async execute(interaction) {
        ...
        // https://keyauth.win/api/seller/?sellerkey=sellerkey&type=delappsub&name=name
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=delappsub&name=${name}`)
            .then(res => res.json())
            .then(json => {
                if (json.success) {
