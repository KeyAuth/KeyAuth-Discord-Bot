...
module.exports = {
    ...
    async execute(interaction) {
        ...
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=killall`)
            ...
            .then(json => {
                if (json.success) {
                    interaction.editReply({ embeds: [new EmbedBuilder().setTitle("Successfully Killed All Sessions").setColor(Colors.Green).setTimestamp().setFooter({ text: "mazkdevf_bot Discord Bot" })], ephemeral: ephemeral })
                } else {
