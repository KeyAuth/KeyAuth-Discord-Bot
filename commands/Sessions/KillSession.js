...
module.exports = {
    data: new SlashCommandBuilder()
        .setName("killsession")
        .setDescription("End Selected Session")
        .addStringOption((option) =>
            option
                .setName("sessid")
                .setDescription("The session id you would like to end.")
                .setRequired(true)
        ),
    async execute(interaction) {
        ...
        let sessid = interaction.options.getString("sessid")

        // https://keyauth.win/api/seller/?sellerkey={sellerkey}&type=kill&sessid={sessid}
        fetch(`https://keyauth.win/api/seller/?sellerkey=${sellerkey}&type=kill&sessid=${sessid}`)
            .then(res => res.json())
            .then(json => {
                if (json.success) {
