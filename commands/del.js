...
module.exports = {
    ...
    async execute(interaction) {
        ...
        if(json.success)
        {
            interaction.editReply({ embeds: [new Discord.EmbedBuilder().setTitle(json.message).addFields([{name: 'Key Deleted:', value: `\`${key}\``}]).setColor(Colors.Green).setTimestamp()], ephemeral: ephemeral})
        }
        else
        {
